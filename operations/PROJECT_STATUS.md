...
| Reconcile post-publication evidence for Windows Defender capability guide PR #43 | Choose Better Tech | In progress |
...
| Cloud Storage cluster approval | Approved | Formal approval recorded at `docs/strategy/cloud-storage-cluster-approval.md`; Proton Drive Review and iCloud vs Google Drive are complete, and Best Secure Cloud Storage is approved for deployment. |
...
| Windows Defender vs Paid Antivirus | Published — production verification recorded | Issue #42; PR #43 merged as `b974fce653fabdd9da10d1f0bf7162d12f70f11a`. Research, 23-claim ledger, independent review, 39/40 score, technical checks, preview QA, production validation, route registration, and sitemap inclusion are recorded. No affiliate link added. Indexing remains subject to Search Console evidence. |
| Optery affiliate readiness | IMPLEMENTED — OWNER ENVIRONMENT CONFIGURATION / PREVIEW VALIDATION REQUIRED | Issue #44; draft PR #45. The serverless `/api/go/optery` redirect reads `OPTERY_AFFILIATE_URL` only at runtime and safely fails closed. The review-only placement, link attributes, test coverage, claim ledger, and independence audit are complete; Preview and Production configuration plus browser validation remain required before merge readiness. |
...
| Optery private runtime configuration | The server-side redirect is implemented but cannot redirect until Vercel holds the private value. | Choose Better Tech | Add `OPTERY_AFFILIATE_URL` in Project Settings for Preview and Production, redeploy Preview, then confirm configuration without sharing the value. |
...
| TBD | Proton VPN review published | First VPN review asset live. |
| TBD | Best VPNs for Beginners guide published | First VPN guide asset live. |
| TBD | Affiliate applications submitted | Initial partner outreach started. |
| 2026-07-02 | Operations documentation framework created | Operational headquarters established. |
