...
export function BestDataRemovalServicesPage() {
  ...
  return <>
    ...
    <Section className="bg-slate-50"><Heading eyebrow="FAQ" title="Data Removal Service Questions" /><div className="mt-8 grid gap-4">{faqItems.map(([question, answer]) => <article key={question} className="rounded-lg border border-slate-200 bg-white p-6 shadow-soft"><h3 className="text-lg font-bold text-slate-950">{question}</h3><p className="mt-3 text-base leading-7 text-slate-700">{answer}</p></article>)}</div><p className="mt-6 text-sm leading-6 text-slate-700">For the stages behind those estimates, see <Link className="font-semibold text-brand-700 hover:text-brand-800" to="/guides/how-long-does-data-removal-take">how long data-removal services take</Link>.</p></Section>
