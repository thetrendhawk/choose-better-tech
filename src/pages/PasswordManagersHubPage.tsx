...
import { passwordManagersHub as hub } from "../data/passwordManagersHub";
    ...
    <ul className="mt-5 grid gap-3 text-sm leading-6 text-slate-700">
      {items.map((item) => (
        <li key={item} className="flex gap-3">
          <AlertCircle className="mt-0.5 shrink-0 text-brand-700" size={17} aria-hidden="true" />
          <span>{item}</span>
        </li>
      ))}
    </ul>
...
export function PasswordManagersHubPage() {
  const breadcrumbSchema = {
    "@context": "https://schema.org",
    "@type": "BreadcrumbList",
    itemListElement: [
      { "@type": "ListItem", position: 1, name: "Home", item: site.url + "/" },
      { "@type": "ListItem", position: 2, name: "Password Manager Decision Hub", item: site.url + hub.path }
    ]
  };
    ...
    <>
      ...
      <section className="border-b border-slate-200 bg-[linear-gradient(135deg,#fbfcff_0%,#ffffff_45%,#eefdf9_100%)] py-16 sm:py-20">
        <Container className="grid gap-10 lg:grid-cols-[minmax(0,1fr)_22rem] lg:items-start">
          <div>
            <p className="text-sm font-semibold uppercase tracking-wide text-brand-700">Password Manager Decision Hub</p>
      ...
      <Section id="choose-by-need" className="bg-slate-50">
        <SectionHeading eyebrow="Decision cards" title="Choose by Need" description="Use the card that sounds closest to your question, then read the linked review or comparison." />
      ...
      <Section className="bg-slate-50">
        <SectionHeading eyebrow="Buyer education" title="Read Before You Choose" description="There is not a full password-manager learning library yet, so this path focuses on the current approved reviews, comparisons, and editorial standards." />
      ...
      <Section>
        <SectionHeading eyebrow="Open questions" title="Details to Verify Before Buying" description="The approved research marks these details as volatile or unresolved. They should be rechecked before active promotion or purchase decisions." />
        <div className="mt-8 rounded-lg border border-slate-200 bg-white p-6 shadow-soft">
          <BulletList items={hub.openQuestions} />
        </div>
      </Section>
      ...
      <Section className="bg-slate-50">
        <div className="grid gap-8 rounded-lg border border-slate-200 bg-white p-6 shadow-soft lg:grid-cols-[1fr_auto] lg:items-center">
          <div>
            ...
            <p className="mt-3 max-w-3xl text-base leading-7 text-slate-700">Choose Better Tech uses approved research, evidence organization, editorial planning, and review before making recommendations. Affiliate status stays separate from editorial conclusions.</p>
          </div>
