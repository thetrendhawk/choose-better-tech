...
export function Navbar() {
  ...
  const mobileItems = [...navItems, { key: "affiliate-disclosure", label: "Affiliate Disclosure", href: "/affiliate-disclosure" }];
  return (
    <header className="sticky top-0 z-40 border-b border-slate-200 bg-white/95 backdrop-blur">
      <Container className="flex min-h-16 items-center justify-between gap-6">
        ...
        <nav className="hidden items-center gap-7 text-sm font-medium text-slate-700 md:flex" aria-label="Primary navigation">
          {mobileItems.map((item) => <NavLink key={item.key} to={item.href} className={({ isActive }) => isActive ? "text-brand-700" : "hover:text-slate-950"}>{item.label}</NavLink>)}
        </nav>
